#!/usr/bin/env node
// scripts/seed-flip-provider.js
// Helper: insert/update data PaymentProvider Flip ke DB (encrypted)
//
// Usage (dari root project):
//   node --env-file=.env scripts/seed-flip-provider.js \
//     --email=akun@flip.id \
//     --token=BEARER_TOKEN_FLIP \
//     --pin=123456

import { encrypt } from '../packages/shared/src/crypto/index.js'
import { getDb, disconnectDb } from '../packages/shared/src/db/index.js'
...
if (!email || !token || !pin) {
  console.error('Usage: node --env-file=.env scripts/seed-flip-provider.js --email=X --token=Y --pin=Z')
  console.error('')
...
try {
  ...
  // tokenExpiresAt di-set 1 jam dari sekarang → akan di-refresh otomatis saat digunakan
