...
export default function AdminMerchantsPage() {
  ...
  const [selected, setSelected] = useState(null)  // merchant detail modal
  ...
  useEffect(() => {
    api.get('/v1/admin/plans').then(r => setPlans(r.data || [])).catch(() => {})
  }, [])
  ...
  const openDetail = async (c) => {
    ...
    try {
      const res = await api.get(`/v1/admin/clients/${c.id}`)
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>Tidak ada data</td></tr>
              ) : clients.map(c => {
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td style={{ fontWeight: 600 }}>Rp {fmt(c.balance_available)}</td>
                    <td className="text-sm text-muted">{c.invoice_count}</td>
                    <td className="text-sm text-muted">{new Date(c.created_at).toLocaleDateString('id-ID')}</td>
                    <td>
              ...
              <>
                ...
                {selected.recent_invoices?.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.78rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 8 }}>Invoice Terakhir</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {selected.recent_invoices.slice(0, 5).map(inv => (
                        <div key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.78rem', padding: '6px 10px', background: 'rgba(255,255,255,0.02)', borderRadius: 6 }}>
                ...
                <div className="modal-actions">
                  ...
                  <button
                    className={`btn ${selected.status === 'active' ? 'btn-danger' : 'btn-primary'}`}
        ...
        <div className="modal-overlay" onClick={() => setShowPlanModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 380 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
              {plans.map(plan => (
                <button key={plan.id} className="btn btn-ghost"
