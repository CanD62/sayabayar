...
const STATUS_CONFIG = {
  ...
  processing: { label: 'Processing', cls: 'badge-info',    Icon: AlertCircle },
  processed:  { label: 'Processed',  cls: 'badge-success', Icon: CheckCircle2 },
  failed:     { label: 'Gagal',      cls: 'badge-danger',  Icon: XCircle },
...
export default function AdminWithdrawalsPage() {
  ...
  const handleProcess = async (id) => {
    setActionId(id)
    try {
      await api.post(`/v1/admin/withdrawals/${id}/process`)
      toast.success('Withdrawal dijadwalkan untuk diproses')
      load()
    } catch (e) {
  ...
  const handleReject = async () => {
    ...
    try {
      await api.post(`/v1/admin/withdrawals/${rejectTarget.id}/reject`, {
        reason: rejectReason || 'Ditolak oleh admin'
      })
      toast.success('Withdrawal ditolak, saldo dikembalikan ke merchant')
          ...
          <table>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>Tidak ada withdrawal</td></tr>
              ) : withdrawals.map(w => {
                ...
                return (
                  <tr key={w.id}>
                    <td>
                      <div style={{ fontWeight: 600 }}>{w.client_name}</div>
                      <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{w.client_email}</div>
                    </td>
                    <td>
                      <div style={{ fontWeight: 700 }}>Rp {fmt(w.amount_received)}</div>
                      <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>Didebit: Rp {fmt(w.amount)}</div>
                    </td>
                    <td>
                      <div style={{ fontWeight: 600 }}>{w.destination_bank} {w.destination_account}</div>
                      <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{w.destination_name}</div>
                    </td>
                    <td>
                      <span className={`badge ${s.cls}`}>{s.label}</span>
                      {w.rejection_reason && (
                        <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 2 }}>"{w.rejection_reason}"</div>
                      )}
                      {w.flip_trx_id && (
                        <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 2 }}>Flip: {w.flip_trx_id}</div>
                      )}
                    </td>
                    <td className="text-sm text-muted">{new Date(w.requested_at).toLocaleString('id-ID')}</td>
                    <td>
                      {canAct && (
        ...
        <div className="modal-overlay" onClick={() => setRejectTarget(null)}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            ...
            <div className="form-group">
              ...
              <input type="text" className="form-input" placeholder="Rekening tidak valid, dll..." value={rejectReason}
                onChange={e => setRejectReason(e.target.value)} />
            </div>
            <div style={{ fontSize: '0.78rem', color: '#f59e0b', marginBottom: 16 }}>
              ⚠ Saldo Rp {fmt(rejectTarget.amount)} akan dikembalikan ke merchant.
            </div>
            <div className="modal-actions">
              <button className="btn btn-ghost" onClick={() => setRejectTarget(null)}>Batal</button>
              <button className="btn btn-danger" onClick={handleReject} disabled={!!actionId}>
                {actionId ? 'Memproses...' : 'Ya, Tolak & Kembalikan Saldo'}
              </button>
            </div>
          </div>
        </div>
